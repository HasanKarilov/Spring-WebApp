...
**Очищение сессии.**
Делается с помощью класса SessionStatus, вызовом его метода setComplete(). Мы говорим все очистить и все обьекты которые были добавлены спомощью @SessionAttributes будут уничтожены.

Model и ModelMap это два совершенно одинаковых обьекта. Model - это интерфейс, а ModelMap это его реализация. Рекомендуется использовать ModelMap.